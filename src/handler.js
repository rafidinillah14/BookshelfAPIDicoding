...
const addBookHandler = (request, h) => {
  ...
  const isSucces = books.filter((item) => item.id === id).length > 0
  if (isSucces) {
    const response = h.response({
      status: 'success',
...
const getAllBooksHandler = (request, h) => {
  ...
  if (name) {
    const fBookName = books.filter((book) => book.name.toLowerCase().includes(name.toLowerCase()))
    const response = h.response({
      status: 'success',
      data: {
        books: fBookName.map((book) => ({
          id: book.id,
          name: book.name,
